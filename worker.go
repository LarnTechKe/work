...
type worker struct {
	...
	contextValue reflect.Value // pre-allocated, reused across jobs
...
func newWorker(namespace string, poolID string, pool *redis.Pool, contextType reflect.Type, middleware []*middlewareHandler, jobTypes map[string]*jobType, sleepBackoffs []int64) *worker {
	...
	w.contextValue = reflect.New(w.contextType)
...
func (w *worker) processJob(job *Job) {
	...
	if jt == nil {
		...
		w.contextValue.Elem().Set(reflect.Zero(w.contextType))
		runErr = runJobWithCtx(job, w.contextValue, w.middleware, jt)
	...
	if runErr != nil {
		...
		historyStatus = w.resolveHistoryStatus(jt, job)
	} else {
		historyStatus = "success"
	}
	history := recordHistory(w, job, historyStatus)
	w.removeJobFromInProgress(job, fate, history)
}
...
func (w *worker) removeJobFromInProgress(job *Job, fate terminateOp, history terminateOp) {
	conn := w.pool.Get()
	defer conn.Close()
...
func terminateAndRetry(w *worker, jt *jobType, job *Job) terminateOp {
	...
	return func(conn redis.Conn) {
		conn.Send("ZADD", redisKeyRetry(w.namespace), nowEpochSeconds()+calcJobBackoff(jt, job), rawJSON)
	}
}
...
func (w *worker) jobFate(jt *jobType, job *Job) terminateOp {
	maxRetries := calcJobMaxRetries(jt, job)

	failsRemaining := maxRetries - job.Fails
	if failsRemaining > 0 {
		return terminateAndRetry(w, jt, job)
	}

	if jt != nil && jt.SkipDead {
		return terminateOnly
	}
	return terminateAndDead(w, job)
}

func (w *worker) resolveHistoryStatus(jt *jobType, job *Job) string {
	maxRetries := calcJobMaxRetries(jt, job)
	if maxRetries-job.Fails > 0 {
		return "retry"
	}
	return "dead"
}

func recordHistory(w *worker, job *Job, status string) terminateOp {
	now := nowEpochSeconds()
	hj := &HistoryJob{
		CompletedAt: now,
		Status:      status,
	...
	rawJSON, err := json.Marshal(hj)
	if err != nil {
		logError("worker.record_history.serialize", err)
		return terminateOnly
	}
	return func(conn redis.Conn) {
		conn.Send("ZADD", redisKeyHistory(w.namespace), now, rawJSON)
		conn.Send("ZADD", redisKeyHistoryByName(w.namespace, job.Name), now, rawJSON)
		conn.Send("HSET", redisKeyHistoryIndex(w.namespace), job.ID, rawJSON)
	}
}
