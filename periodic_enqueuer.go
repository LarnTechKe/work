...
func (pe *periodicEnqueuer) enqueue() error {
	...
	// Enqueue code-defined periodic jobs
	for _, pj := range pe.periodicJobs {
		if err := pe.enqueuePeriodicJob(conn, pj, nowTime, horizon); err != nil {
			return err
		}
	}

	// Enqueue Redis-stored periodic schedules
	if err := pe.enqueueRedisSchedules(conn, nowTime, horizon); err != nil {
		logError("periodic_enqueuer.enqueue_redis_schedules", err)
		// Don't return error - Redis schedules failing shouldn't block code-defined ones
	}
...
func (pe *periodicEnqueuer) enqueuePeriodicJob(conn redis.Conn, pj *periodicJob, nowTime time.Time, horizon time.Time) error {
...
func (pe *periodicEnqueuer) enqueueRedisSchedules(conn redis.Conn, nowTime time.Time, horizon time.Time) error {
	vals, err := redis.StringMap(conn.Do("HGETALL", redisKeyPeriodicSchedules(pe.namespace)))
	if err != nil {
		if err == redis.ErrNil {
			return nil
		}
	...
	for _, v := range vals {
		...
		if err := json.Unmarshal([]byte(v), &s); err != nil {
			logError("periodic_enqueuer.parse_redis_schedule", err)
			continue
		}
		...
		schedule, err := p.Parse(s.Spec)
		if err != nil {
			logError("periodic_enqueuer.parse_cron_spec", err)
			continue
		}
		...
		if err := pe.enqueuePeriodicJob(conn, pj, nowTime, horizon); err != nil {
			logError("periodic_enqueuer.enqueue_redis_periodic_job", err)
			continue
		}
	}
