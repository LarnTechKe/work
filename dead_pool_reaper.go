...
func (r *deadPoolReaper) reap() error {
	...
	// Clean up history entries older than 15 days
	if err := r.cleanHistory(); err != nil {
		logError("dead_pool_reaper.clean_history", err)
	}
...
const historyRetentionSeconds = 15 * 24 * 60 * 60 // 15 days

func (r *deadPoolReaper) cleanHistory() error {
	conn := r.pool.Get()
	defer conn.Close()

	cutoff := nowEpochSeconds() - int64(historyRetentionSeconds)

	// Clean main history ZSET
	if _, err := conn.Do("ZREMRANGEBYSCORE", redisKeyHistory(r.namespace), "-inf", cutoff); err != nil {
		return err
	}

	// Clean per-job-name history ZSETs
	jobNames, err := redis.Strings(conn.Do("SMEMBERS", redisKeyKnownJobs(r.namespace)))
	if err != nil && err != redis.ErrNil {
		return err
	}
	for _, jobName := range jobNames {
		if _, err := conn.Do("ZREMRANGEBYSCORE", redisKeyHistoryByName(r.namespace, jobName), "-inf", cutoff); err != nil {
			logError("dead_pool_reaper.clean_history_by_name", err)
		}
	}

	// Clean the history index HASH via HSCAN
	var cursor int64
	for {
		values, err := redis.Values(conn.Do("HSCAN", redisKeyHistoryIndex(r.namespace), cursor, "COUNT", 100))
		if err != nil {
			return err
		}
		newCursor, _ := redis.Int64(values[0], nil)
		items, _ := redis.StringMap(values[1], nil)
		for jobID, jsonStr := range items {
			var hj HistoryJob
			if err := json.Unmarshal([]byte(jsonStr), &hj); err != nil {
				continue
			}
			if hj.CompletedAt < cutoff {
				conn.Send("HDEL", redisKeyHistoryIndex(r.namespace), jobID)
			}
		}
