...
func TestWebUIQueues(t *testing.T) {
	...
	request, _ := http.NewRequest("GET", "/api/queues", nil)
...
func TestWebUIWorkerPools(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	request, _ := http.NewRequest("GET", "/api/worker_pools", nil)
	s.ServeHTTP(recorder, request)
	assert.Equal(t, 200, recorder.Code)
...
func TestWebUIBusyWorkers(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	request, _ := http.NewRequest("GET", "/api/busy_workers", nil)
	s.ServeHTTP(recorder, request)
	assert.Equal(t, 200, recorder.Code)
	...
	enqueuer.Enqueue("wat", nil)
	wgroup2.Wait()
	time.Sleep(5 * time.Millisecond) // need to let observer process
...
func TestWebUIRetryJobs(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	request, _ := http.NewRequest("GET", "/api/retry_jobs", nil)
	s.ServeHTTP(recorder, request)
	assert.Equal(t, 200, recorder.Code)
	var res struct {
...
func TestWebUIScheduledJobs(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	request, _ := http.NewRequest("GET", "/api/scheduled_jobs", nil)
	s.ServeHTTP(recorder, request)
	assert.Equal(t, 200, recorder.Code)
	var res struct {
		Count int64 `json:"count"`
...
func TestWebUIDeadJobs(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	request, _ := http.NewRequest("GET", "/api/dead_jobs", nil)
	s.ServeHTTP(recorder, request)
	assert.Equal(t, 200, recorder.Code)
	var res struct {
		Count int64 `json:"count"`
	...
	recorder = httptest.NewRecorder()
	request, _ = http.NewRequest("POST", fmt.Sprintf("/api/delete_dead_job/%d/%s", diedAt0, id0), nil)
	s.ServeHTTP(recorder, request)
	assert.Equal(t, 200, recorder.Code)

	recorder = httptest.NewRecorder()
	request, _ = http.NewRequest("POST", fmt.Sprintf("/api/retry_dead_job/%d/%s", diedAt1, id1), nil)
	s.ServeHTTP(recorder, request)
	assert.Equal(t, 200, recorder.Code)
...
func TestWebUIDeadJobsDeleteRetryAll(t *testing.T) {
	...
	recorder = httptest.NewRecorder()
	request, _ = http.NewRequest("POST", "/api/retry_all_dead_jobs", nil)
	s.ServeHTTP(recorder, request)
	assert.Equal(t, 200, recorder.Code)
	...
	// Now delete them:
	recorder = httptest.NewRecorder()
	request, _ = http.NewRequest("POST", "/api/delete_all_dead_jobs", nil)
	s.ServeHTTP(recorder, request)
	assert.Equal(t, 200, recorder.Code)
...
func TestWebUIDashboard(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	request, _ := http.NewRequest("GET", "/api/dashboard", nil)
	s.ServeHTTP(recorder, request)
	assert.Equal(t, 200, recorder.Code)
