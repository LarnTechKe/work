...
// runJobWithCtx is like runJob but reuses a pre-allocated context value.
// The caller must zero the context before calling.
func runJobWithCtx(job *Job, ctx reflect.Value, middleware []*middlewareHandler, jt *jobType) (returnError error) {
	currentMiddleware := 0
	maxMiddleware := len(middleware)
	...
	next = func() error {
		if currentMiddleware < maxMiddleware {
			...
			res := mw.DynamicMiddleware.Call([]reflect.Value{ctx, reflect.ValueOf(job), reflect.ValueOf(next)})
			x := res[0].Interface()
			if x == nil {
				return nil
			}
		...
		res := jt.DynamicHandler.Call([]reflect.Value{ctx, reflect.ValueOf(job)})
		x := res[0].Interface()
		if x == nil {
			return nil
		}
