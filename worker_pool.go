...
func (wp *WorkerPool) PeriodicallyEnqueue(spec string, jobName string) *WorkerPool {
	return wp.PeriodicallyEnqueueWithArgs(spec, jobName, nil)
}

// PeriodicallyEnqueueWithArgs works like PeriodicallyEnqueue but also sets
// the provided args on each enqueued job instance.
...
func (wp *WorkerPool) Daily(jobName string, hour, minute int) *WorkerPool {
	...
	return wp.PeriodicallyEnqueue(spec, jobName)
}
...
func (wp *WorkerPool) Every(jobName string, d time.Duration) *WorkerPool {
	spec := fmt.Sprintf("@every %s", d.String())
	return wp.PeriodicallyEnqueue(spec, jobName)
}
